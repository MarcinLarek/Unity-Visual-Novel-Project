...
namespace TESTING
{
    public class CMD_DatabaseExtension_Examples : CMD_DatabaseExtension
    {
        new public static void Extend(CommandDatabase database)
        {
            ...
            database.AddCommand("lamba", new Action(() => { Debug.Log("Printing a default message to console from lambda command"); }));
            database.AddCommand("lamba_1p", new Action<string>((arg) => { Debug.Log($"Log user Lambda Message: '{arg}'"); }));
            database.AddCommand("lamba_mp", new Action<string[]>((args) => { Debug.Log(string.Join(", ", args)); }));
        ...
        private static IEnumerator MoveCharacter(string direction)
        {
            ...
            //Get the variables I need. This wouyld be defined somewhere else.
